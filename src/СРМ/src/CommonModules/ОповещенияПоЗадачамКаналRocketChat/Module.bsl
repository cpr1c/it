...
//		ТекстИзменений="---" + Символы.ПС + Символы.ПС;
		Для Каждого ТекИзменение Из Изменения Цикл
			ТекстИзменений=ТекстИзменений + "- Параметр *" + ТекИзменение.Ключ + "* изменился с _"
...
Функция ТекстКомментарияПреобразованный(Инициатор, Знач КомментарийИзБазы) Экспорт
	ТекстКомментария=КомментарийИзБазы;
	Если ТипЗнч(Инициатор) = Тип("СправочникСсылка.КомментарииЗадач") Тогда
		Формат=Инициатор.ТекстСообщенияФормат;
	ИначеЕсли ТипЗнч(Инициатор) = Тип("ДокументСсылка.Задача") Тогда
		Формат=Инициатор.СодержаниеФормат;
	КонецЕсли;

	Если Формат = Перечисления.ВариантыОформленияТекста.HTML Тогда
