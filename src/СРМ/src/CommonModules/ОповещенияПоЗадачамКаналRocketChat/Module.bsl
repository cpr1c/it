...
Функция ТекстКомментарияПреобразованный(Сообщение, Знач КомментарийИзБазы) Экспорт
	ТекстКомментария=КомментарийИзБазы;
	Если ТипЗнч(Сообщение.ДополнительныеПараметры.Инициатор) = Тип("СправочникСсылка.КомментарииЗадач") Тогда
		Если Сообщение.ДополнительныеПараметры.Инициатор.ТекстСообщенияФормат
			= Перечисления.ВариантыОформленияТекста.HTML Тогда
