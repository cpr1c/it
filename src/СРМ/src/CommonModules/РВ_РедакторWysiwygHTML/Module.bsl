...
Функция ТекстПоляHTMLРедактора() Экспорт
	Текст=ПолучитьОбщийМакет("РВ_ТекстПоляРедактора").ПолучитьТекст();
	Возврат Текст;
...
Процедура ВставитьПолеРедактированияТекстаНаФорму(Форма, ГруппаЭлементовТекста, ПутьКДанным) Экспорт
	...
	ОписаниеЭлемента.Действия.Вставить("ПриНажатии", "РВ_РедакторWysiwyg_Подключаемый_ДокументПриНажатииПоляРедактированияКомментария");
...
Процедура ЗаписатьНовыеПрисоединенныеФайлы(Форма, ВладелецФайлов,ПутьКДанным,Предмет=Неопределено) Экспорт
	...
	Для Каждого СтрокаФайла Из ДанныеПрисоединенныхФайлов Цикл
		...
		РВ_РедакторWysiwygHTMLПереопределяемый.ЗаписатьПрисоединенныйФайлВБазу(ПараметрыФайла,СтрокаФайла.Адрес,Форма, ПутьКДанным,Предмет);
	КонецЦикла;
КонецПроцедуры
