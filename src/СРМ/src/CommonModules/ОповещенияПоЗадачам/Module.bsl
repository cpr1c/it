...
Процедура СформироватьСообщениеИОтправитьОповещение(МенеджерЗаписи, Ошибка, ДатаОтправки) 

	ДанныеСобытия=МенеджерЗаписи.ДанныеСобытия.Получить();
	Если ДанныеСобытия.ЭтоИзменениеИзВнешнихИсточников = Истина Тогда
		ДанныеСобытия.Вставить("Автор", МенеджерЗаписи.ИнициаторСобытия.Основание.ОтправительКонтакт);
		Если Не ЗначениеЗаполнено(ДанныеСобытия.Автор) Тогда
			ДанныеСобытия.Вставить("Автор", "" + МенеджерЗаписи.ИнициаторСобытия.Основание.ОтправительПредставление + "<"
				+ МенеджерЗаписи.ИнициаторСобытия.Основание.ОтправительАдрес + ">");
		КонецЕсли;
	Иначе
	...
	Если ТипЗнч(МенеджерЗаписи.ИнициаторСобытия) = Тип("СправочникСсылка.КомментарииЗадач") Тогда
		ДанныеСобытия.Вставить("Комментарий", МенеджерЗаписи.ИнициаторСобытия.ТекстСообщения);
		ДанныеСобытия.Вставить("КомментарийHTML", МенеджерЗаписи.ИнициаторСобытия.ТекстСообщенияHTML);
		ДанныеСобытия.Вставить("ФорматКомментария", РедакторКомментария.ФорматТекстаРедактирования(
			МенеджерЗаписи.ИнициаторСобытия.ТекстСообщенияФормат));
	ИначеЕсли ДанныеСобытия.ЭтоНовый Тогда
		ДанныеСобытия.Вставить("Комментарий", МенеджерЗаписи.ИнициаторСобытия.Содержание);
		ДанныеСобытия.Вставить("КомментарийHTML", МенеджерЗаписи.ИнициаторСобытия.СодержаниеHTML);
		ДанныеСобытия.Вставить("ФорматКомментария", РедакторКомментария.ФорматТекстаРедактирования(
			МенеджерЗаписи.ИнициаторСобытия.СодержаниеФормат));
	Иначе
	...
	ДанныеСобытия.Комментарий = СтрЗаменить(ДанныеСобытия.Комментарий, "<img>", "");
	...
	Для Каждого КлючЗначение Из ДанныеСобытия.Изменения Цикл
		Если КлючевыеРеквизиты.Найти(КлючЗначение.Ключ) = Неопределено И Не СтрНачинаетсяС(КлючЗначение.Ключ,
...
Процедура ОтправитьОповещение(ИдентификаторТранзакции, Идентификатор, Событие, ИнициаторСобытия) Экспорт
	...
	Попытка
		СформироватьСообщениеИОтправитьОповещение(МенеджерЗаписи, Ошибка, ДатаОтправки);
	Исключение
		Ошибка = ОписаниеОшибки();
	КонецПопытки;
...
Процедура ЗаполнитьПолучателейСообщенияОповещения(Сообщение, ДанныеСобытия)
	...
	Для Каждого КлючЗначение Из ДанныеСобытия.ИзмененияКлючевыхРеквизитов Цикл
		Если СтрНачинаетсяС(КлючЗначение.Ключ, "ДополнительныеИсполнители") Тогда
			Попытка
				УникальныйИдентификаторСсылки = СтрЗаменить(КлючЗначение.Ключ, "ДополнительныеИсполнители", "");
				УникальныйИдентификаторСсылки = Новый УникальныйИдентификатор(СтрЗаменить(УникальныйИдентификаторСсылки,
																						  "_",
																						  "-"));
				СсылкаДопИсполнитель = Справочники.Пользователи.ПолучитьСсылку(УникальныйИдентификаторСсылки);
				
			Исключение
				Продолжить;
			КонецПопытки;
			
			ДобавитьПотенциальногоПолучателя(МассивПотенциальныхПолучателей, СсылкаДопИсполнитель, ДанныеСобытия);
		КонецЕсли;
	КонецЦикла;
	...
	Если ТипЗнч(Источник) = Тип("ДокументОбъект.Задача") Тогда
		...
		Иначе
			...
			Для Каждого КлючЗначение Из ДанныеТранзакцииОповещений.Изменения Цикл
				Если КлючевыеРеквизиты.Найти(КлючЗначение.Ключ) <> Неопределено Или СтрНачинаетсяС(КлючЗначение.Ключ,
																								   "ДополнительныеИсполнители") Тогда
