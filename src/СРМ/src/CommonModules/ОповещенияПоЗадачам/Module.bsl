...
Процедура ПриФормированииСообщения(Сообщение, НазначениеШаблона, ПредметСообщения, ПараметрыШаблона) Экспорт
	...
	Если НазначениеШаблона = "ОповещениеОбИзмененииЗадачиДляКонтакта" Тогда
		...
		Сообщение.ЗначенияРеквизитов["ТекстКомментария"]=МодульКаналаОповещения.ТекстКомментарияПреобразованный(
			Сообщение.ДополнительныеПараметры.Инициатор, Сообщение.ДополнительныеПараметры.Комментарий, Сообщение.ДополнительныеПараметры);
			...
			Для Каждого КлючЗначение Из ПараметрыЗадача Цикл
				Если КлючЗначение.Ключ = "Содержание" Тогда
					ПараметрыЗадача[КлючЗначение.Ключ]=МодульКаналаОповещения.ТекстКомментарияПреобразованный(
						Сообщение.ДополнительныеПараметры.Задача, Задача[КлючЗначение.Ключ]);
				Иначе
...
Процедура ПриЗаписиОбъектаФормированиеДанныхОповещений(Источник, Отказ) Экспорт
	...
	КонецЕсли;
	ДанныеТранзакцииОповещений.Вставить("УстанавливатьОтправителемУведомленияАвтораСобытия", НастройкаОповещения.УстанавливатьОтправителемУведомленияАвтораСобытия);
...
Процедура ОтправитьОповещение(ИдентификаторТранзакции, Идентификатор, Событие, ИнициаторСобытия) Экспорт
	...
	Если ТипЗнч(ИнициаторСобытия) = Тип("СправочникСсылка.КомментарииЗадач") Тогда
		...
		ДанныеСобытия.Вставить("ФорматКомментария",  РедакторКомментария.ФорматТекстаРедактирования(ИнициаторСобытия.ТекстСообщенияФормат));
	ИначеЕсли ДанныеСобытия.ЭтоНовый Тогда
		...
		ДанныеСобытия.Вставить("ФорматКомментария", РедакторКомментария.ФорматТекстаРедактирования(ИнициаторСобытия.СодержаниеФормат));
	Иначе
	...
	НастройкиОтправкиОповещения.Вставить("УстанавливатьОтправителемУведомленияАвтораСобытия",ДанныеСобытия.УстанавливатьОтправителемУведомленияАвтораСобытия);
	...
	Иначе
		МодульКаналаОповещения.ВыполнитьОтправкуСообщения(Сообщение, МенеджерЗаписи.УчетнаяЗапись, Ошибка, НастройкиОтправкиОповещения);
