...
Процедура ПередЗаписьюОбъектаФормированиеДанныхДляОповещений(Источник,Отказ) Экспорт
	...
	Источник.ДополнительныеСвойства.Вставить("Изменения", ИзмененияОбъектаПередЗаписью(Источник));
	Источник.ДополнительныеСвойства.Вставить("Метаданные", Источник.Метаданные());
	Источник.ДополнительныеСвойства.Вставить("ЭтоЗадача", ТипЗнч(Источник)=Тип("ДокументОбъект.Задача"));
	Источник.ДополнительныеСвойства.Вставить("События", Новый Массив);
...
Процедура ПриЗаписиОбъектаФормированиеДанныхОповещений(Источник, Отказ) Экспорт
	...
	Иначе
		Проект=Источник.Задача.Проект;
	КонецЕсли;
	
	Партнер=Проект.Партнер;
	
	
	Если ЗначениеЗаполнено(Проект.НастройкаОповещенияПоЗадачам) Тогда
		НастройкаОповещения=Проект.НастройкаОповещенияПоЗадачам;
	ИначеЕсли ЗначениеЗаполнено(Партнер.НастройкаОповещенияПоЗадачам) Тогда 
		НастройкаОповещения=Партнер.НастройкаОповещенияПоЗадачам;
	Иначе
...
Функция ИзмененияОбъектаПередЗаписью(Источник)
	...
	Для Каждого Реквизит Из Метаданныеобъекта.Реквизиты Цикл
		Если Источник[Реквизит.Имя]<>ПредыдущийОбъект[Реквизит.Имя] Тогда
			СтруктураИзменений.Вставить(Реквизит.Имя, Новый Структура("ПредыдущееЗначение, Значение", ПредыдущийОбъект[Реквизит.Имя], Источник[Реквизит.Имя]));
		КонецЕсли;	
	КонецЦикла;
