...
Процедура ПерейтиНаВерсию_1_6_9_1() Экспорт
	//Проводки по новому регистру выработка исполнителей
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Задача.Ссылка,
	...
	|ГДЕ
	|	Задача.ДатаЗакрытия <> &ДатаПустая
	|	И Задача.Статус.Вид = &Вид";
	Запрос.УстановитьПараметр("ДатаПустая", '00010101');
	Запрос.УстановитьПараметр("Вид", Перечисления.ВидыСтатусовЗадач.Закрыта);
	
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Набор = РегистрыНакопления.ВыработкаИсполнителей.СоздатьНаборЗаписей();
		Набор.Отбор.Регистратор.Установить(Выборка.Ссылка, Истина);
		Набор.Прочитать();
		...
		Запись.Период = Выборка.ДатаЗакрытия;
		Запись.Исполнитель = Выборка.Исполнитель;
		Запись.Количество = Выборка.ОценкаТрудозатрат;
