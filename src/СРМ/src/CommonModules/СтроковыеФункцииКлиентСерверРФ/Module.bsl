...
Функция НайтиПодстрокуИВыделить(СтрокаНачальная, ТекстПоиска) Экспорт
	Если Не ЗначениеЗаполнено(ТекстПоиска) Тогда
		Возврат СтрокаНачальная;
	КонецЕсли;

	СтрокаИзначальная=СтрокаНачальная;
	СтрокаДляПоиска=НРег(СтрокаИзначальная);
	НоваяСтрока="";
	ДлинаСтрокиПоиска=СтрДлина(ТекстПоиска);

	ПозицияСимвола=СтрНайти(СтрокаДляПоиска, ТекстПоиска);
	Пока ПозицияСимвола > 0 Цикл
		ФорматированнаяСтрокаПоиска=Новый ФорматированнаяСтрока(Сред(СтрокаИзначальная, ПозицияСимвола,
			ДлинаСтрокиПоиска), Новый Шрифт(, , Истина), WebЦвета.СинийСоСтальнымОттенком);
		НоваяСтрока=Новый ФорматированнаяСтрока(НоваяСтрока, Лев(СтрокаИзначальная, ПозицияСимвола - 1),
			ФорматированнаяСтрокаПоиска);

		СтрокаИзначальная=Сред(СтрокаИзначальная, ПозицияСимвола + ДлинаСтрокиПоиска);
		СтрокаДляПоиска=НРег(СтрокаИзначальная);
	...
	Если ЗначениеЗаполнено(НоваяСтрока) Тогда
		НоваяСтрока=Новый ФорматированнаяСтрока(НоваяСтрока, СтрокаИзначальная);
	КонецЕсли;
