...
Процедура СоздатьВременныйКаталогПослеПолученияВременногоКаталога(ИмяКаталогаВременныхФайлов, Контекст) Экспорт 
	...
	ИмяКаталога = "v8_" + СтрЗаменить(Новый УникальныйИдентификатор,"-","_");
	
	Если Не ПустаяСтрока(Расширение) Тогда 
		ИмяКаталога = ИмяКаталога + "." + Расширение;
	КонецЕсли;
	
	Если СтрЗаканчиваетсяНа(ИмяКаталогаВременныхФайлов, ПолучитьРазделительПути()) Тогда
		НачатьСозданиеКаталога(Оповещение, ИмяКаталогаВременныхФайлов + ИмяКаталога);
	Иначе
		НачатьСозданиеКаталога(Оповещение, ИмяКаталогаВременныхФайлов + ПолучитьРазделительПути()+ ИмяКаталога);
	КонецЕсли;
