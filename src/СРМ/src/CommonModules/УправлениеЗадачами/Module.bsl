...
Функция ИзмененияЗадачиДляПоляИстории(Задача)
	...
	|	ИзмененияЗадач.Автор,
	|	ИзмененияЗадач.ПредыдущееЗначение,
	|	ИзмененияЗадач.НовоеЗначение,
	|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ИзмененияЗадач.Автор) КАК АвторПредставление,
	...
	Пока Выборка.Следующий() Цикл
		...
		СтруктураИзмененияПоЗадаче.Вставить("date", Выборка.Дата);
		СтруктураИзмененияПоЗадаче.Вставить("author", Выборка.АвторПредставление);
		СтруктураИзмененияПоЗадаче.Вставить("field", Выборка.Реквизит);
		СинонимРеквизита="";
		...
		КонецЕсли;
		СтруктураИзмененияПоЗадаче.Вставить("fieldView", СинонимРеквизита);
		Если ЗначениеЗаполнено(Выборка.ПредыдущееЗначение) Тогда
			СтруктураИзмененияПоЗадаче.Вставить("value", Выборка.ПредыдущееЗначениеПредставление);
		Иначе
			СтруктураИзмененияПоЗадаче.Вставить("value", Неопределено);
		КонецЕсли;
		Если ЗначениеЗаполнено(Выборка.НовоеЗначение) Тогда
			СтруктураИзмененияПоЗадаче.Вставить("newValue", Выборка.НовоеЗначениеПредставление);
		Иначе
			СтруктураИзмененияПоЗадаче.Вставить("newValue", Неопределено);
		КонецЕсли;
