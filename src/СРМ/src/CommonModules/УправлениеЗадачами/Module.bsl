...
Процедура ЗаписатьПрисоединенныеФайлыПоДаннымПрисоединенныхФайловРедакторов(Задача, ДанныеПрисоединенныхФайлов,
	...
	Для Каждого СтрокаФайла Из ДанныеПрисоединенныхФайлов Цикл
		...
		СтрокаФайла.Ссылка = ПараметрыФайла.Ссылка;
	КонецЦикла;
КонецПроцедуры
...
Процедура ЗаписатьПрисоединенныйФайлВБазу(ПараметрыФайла, АдресФайлаВоВременномХранилище, Предмет = Неопределено) Экспорт
	...
	ПараметрыФайла.Вставить("Ссылка", ПрисоединенныйФайл);
...
// Изменения объекта перед записью.
// 
// Параметры:
//  Источник - ДокументОбъект.Задача
//  ВПустыеЗначенияУстанавливатьСтрокуПусто - Булево -  В пустые значения устанавливать строку пусто
// 
// Возвращаемое значение:
//  Структура -  Изменения объекта перед записью
Функция ИзмененияОбъектаПередЗаписью(Источник, ВПустыеЗначенияУстанавливатьСтрокуПусто = Истина) Экспорт
	...
	ПредыдущийОбъект=Источник.Ссылка.ПолучитьОбъект(); //ДокументОбъект.Задача
	...
	Если ТипЗнч(Источник.Ссылка) = Тип("ДокументСсылка.Задача") Тогда
		ДопИсполнители = Новый Соответствие;
		
		Для Каждого Стр Из ПредыдущийОбъект.ДополнительныеИсполнители Цикл
			ДопИсполнители.Вставить(Стр.Исполнитель);
		КонецЦикла;
		Для Каждого Стр Из Источник.ДополнительныеИсполнители Цикл
			ДопИсполнители.Вставить(Стр.Исполнитель);
		КонецЦикла;
		
		Для Каждого КлючЗначение Из ДопИсполнители Цикл
			СтруктураПоиска = Новый Структура;
			СтруктураПоиска.Вставить("Исполнитель", КлючЗначение.Ключ);
						
			СтрокиПредыдущегоОбъекта = ПредыдущийОбъект.ДополнительныеИсполнители.НайтиСтроки(СтруктураПоиска);
			
			ПредыдущееЗначение = 0;
			Если СтрокиПредыдущегоОбъекта.Количество() >0 Тогда
				ПредыдущееЗначение = СтрокиПредыдущегоОбъекта[0].ОценкаТрудозатрат;
			КонецЕсли;
			
			СтрокиОбъекта = Источник.ДополнительныеИсполнители.НайтиСтроки(СтруктураПоиска);
			НовоеЗначение = 0;
			Если СтрокиОбъекта.Количество() >0 Тогда
				НовоеЗначение = СтрокиОбъекта[0].ОценкаТрудозатрат;
			КонецЕсли;
			...
			Изменение=Новый Структура;
			Изменение.Вставить("Синоним", "Оценка трудозатрат доп. исполнителя <" + КлючЗначение.Ключ + ">");
			Изменение.Вставить("ПредыдущееЗначение", ПредыдущееЗначение);
			Изменение.Вставить("Значение", НовоеЗначение);
			Если ВПустыеЗначенияУстанавливатьСтрокуПусто Тогда
				Если Не ЗначениеЗаполнено(Изменение.ПредыдущееЗначение) Тогда
					Изменение.ПредыдущееЗначение="<Пусто>";
				КонецЕсли;
			...
			СтруктураИзменений.Вставить("ДополнительныеИсполнители"
										+ СтрЗаменить(КлючЗначение.Ключ.УникальныйИдентификатор(), "-", "_"), Изменение);
...
Процедура ЗаписатьИзмененияКлючевыхРеквизитовЗадачиПриЗаписи(ЗадачаОбъект) Экспорт
	...
	Для Каждого ТекИзменение Из ЗадачаОбъект.ДополнительныеСвойства.Изменения Цикл
		Если КлючевыеРеквизиты.Найти(ТекИзменение.Ключ) = Неопределено И Не СтрНачинаетсяС(ТекИзменение.Ключ,
																						   "ДополнительныеИсполнители") Тогда
			Продолжить;
		КонецЕсли;
	...
	Пока ВыборкаДата.Следующий() Цикл
		...
		Пока ВыборкаАвтор.Следующий() Цикл
			...
			Пока Выборка.Следующий() Цикл
				...
				ИначеЕсли СтрНачинаетсяС(Выборка.Реквизит, "ДополнительныеИсполнители") Тогда
					УникальныйИдентификаторИсполнителя = СтрЗаменить(Выборка.Реквизит, "ДополнительныеИсполнители", "");
					УникальныйИдентификаторИсполнителя = СтрЗаменить(УникальныйИдентификаторИсполнителя, "_", "-");
					Попытка
						УникальныйИдентификаторИсполнителя = Новый УникальныйИдентификатор(УникальныйИдентификаторИсполнителя);
						СсылкаНаИсполнителя = Справочники.Пользователи.ПолучитьСсылку(УникальныйИдентификаторИсполнителя);
					Исключение
						СсылкаНаИсполнителя = Справочники.Пользователи.ПустаяСсылка();
					КонецПопытки;
					
					СинонимРеквизита = "Оценка трудозатрат доп. исполнителя <" + СсылкаНаИсполнителя + ">";
				КонецЕсли;
