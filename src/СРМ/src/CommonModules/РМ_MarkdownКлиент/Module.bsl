...
Процедура ПередЗаписью(Форма, ПутьКДанным) Экспорт
	ТекстКомментария=ТекстКомментария(Форма, ПутьКДанным);
	
	ИмяЭлементаПоля=РМ_MarkdownКлиентСервер.ПолучитьИмяЭлементаПросмотраКомментария(ПутьКДанным);
	
	ПутьКДаннымБезЛишнего=РМ_MarkdownКлиентСервер.ПолучитьПутьКДаннымБезЛишнего(ПутьКДанным);
	ДанныеПрисоединенныхФайлов=РМ_MarkdownКлиентСервер.ДанныеПрисоединенныхФайлов(Форма, ПутьКДаннымБезЛишнего);
	
	Для Каждого Вложение ИЗ ДанныеПрисоединенныхФайлов Цикл
		ЧастиИмениФайла = РМ_MarkdownКлиентСервер.РазложитьПолноеИмяФайла(Вложение.ИмяФайла);
		РасширениеБезТочки = СтрЗаменить(ЧастиИмениФайла.Расширение, ".", "");
		ИмяБезРасширения = ЧастиИмениФайла.ИмяБезРасширения;
		
		ТекстКомментария=СтрЗаменить(ТекстКомментария, "("+Вложение.ИмяФайла+")", "(cid:"+ИмяБезРасширения+")");	
	КонецЦикла;
	
	ТекстКомментарияHTML=ТекстHTMLПоТекстуMarkdown(Форма.Элементы[ИмяЭлементаПоля], ТекстКомментария);
	
	УстановитьТекстКомментария(Форма, ПутьКДанным, ТекстКомментарияHTML, Истина);
КонецПроцедуры
...
Процедура КомандаMDВставитьИзображениеИзБуфераОбменаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.Форма[ИмяТаблицыФайлов].НайтиСтроки(
		...
		ДобавитьНовыйПрисоединенныйФайл(ИмяФайла, ПутьКФайлу, ДополнительныеПараметры.Форма, ПутьКДаннымБезЛишнего,Адрес);
	Иначе
...
Процедура УстановитьТекстКомментария(Форма, ПутьКДанным, ТекстКомментария, HTML=Ложь)
	СуффиксHTML=?(HTML,"HTML","");
	
	МассивПути=СтрРазделить(ПутьКДанным, ".", Ложь);
	Если МассивПути.Количество() = 1 Тогда
		Форма[МассивПути[0]+СуффиксHTML]=ТекстКомментария;
	ИначеЕсли МассивПути.Количество() = 2 Тогда
		Форма[МассивПути[0]][МассивПути[1]+СуффиксHTML]=ТекстКомментария;
	Иначе
