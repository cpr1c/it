...
//	КонецЕсли;
	...
	Для Каждого ТекТаймер Из Таймеры Цикл
		...
		ИмяГруппыТаймера=Префикс + "ГруппаПредмета_" + Идентификатор;
		...
		Форма[Префикс + "Комментарий_" + Идентификатор] = ТекТаймер.Комментарий;
		Форма[Префикс + "ВидДеятельности_" + Идентификатор] = ТекТаймер.ВидДеятельности;
		Форма[Префикс + "ДатаНачала_" + Идентификатор] = ТекТаймер.ДатаНачала;

		УчетВремениКлиентСервер.ОбновитьОтображениеВремениТаймера(Форма, ТекТаймер, Идентификатор, ТекДата);

	КонецЦикла;

//	ПредметыПоТаймерам = ОбщегоНазначения.ВыгрузитьКолонку(Таймеры, "Предмет", Истина);
//
...
//
//		ГруппаТаймеровПоПредмету = Форма.Элементы.Добавить(Префикс + "ГруппаПредмета_" + ИдПредмета, Тип(
//			"ГруппаФормы"), ГруппаТаймеров);
//		ГруппаТаймеровПоПредмету.Вид         = ВидГруппыФормы.ОбычнаяГруппа;
//		ГруппаТаймеровПоПредмету.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
//		ГруппаТаймеровПоПредмету.Заголовок   = ПредставлениеПредмета;
...
//		ГруппаТаймеровПоПредмету.ОтображениеУправления = ОтображениеУправленияОбычнойГруппы.Картинка;
//
//		ТаймерыПоПредмету = Таймеры.НайтиСтроки(Новый Структура("Предмет", Предмет));
//
...
//
//			ДобавитьЭлементыТекущегоТаймераНаФорму(Форма, ГруппаТаймеровПоПредмету, ДанныеТаймера, Ид, ТекДата);
//
//			Форма[Префикс + "Комментарий_" + Ид] = ДанныеТаймера.Комментарий;
//			Форма[Префикс + "ВидДеятельности_" + Ид] = ДанныеТаймера.ВидДеятельности;
//
//			УчетВремениКлиентСервер.ОбновитьОтображениеВремениТаймера(Форма, ДанныеТаймера, Ид, ТекДата);
//
//		КонецЦикла;
//
//	КонецЦикла;
...
Процедура ДобавитьРеквизитыСДаннымиТаймеровНаФорму(Форма, Таймеры, ТекущиеТаймерыФормы)
	...
	Для Каждого Идентификатор Из ИдентификаторыТаймеров Цикл
		...
		ДобавленныйРеквизит = Новый РеквизитФормы(Префикс + "ДатаНачала_" + Ид, Новый ОписаниеТипов("Дата", , , Новый КвалификаторыДаты(ЧастиДаты.ДатаВремя),));
		МассивДобавляемыхРеквизитов.Добавить(ДобавленныйРеквизит);

	КонецЦикла;

	Для Каждого Идентфикатор Из ТекущиеТаймерыФормы Цикл
		Если МассивИдентификаторовТекущихТАймеров.Найти(Идентификатор) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;

		МассивУдаляемыхРеквизитов.Добавить(Префикс + "ВидДеятельности_" + СтрЗаменить(Идентфикатор, "-", "_"));
		МассивУдаляемыхРеквизитов.Добавить(Префикс + "Комментарий_" + СтрЗаменить(Идентфикатор, "-", "_"));
		МассивУдаляемыхРеквизитов.Добавить(Префикс + "ДатаНачала_" + СтрЗаменить(Идентфикатор, "-", "_"));
	КонецЦикла;
...
Процедура ДобавитьЭлементыТекущегоТаймераНаФорму(Форма, ГруппаТаймеры, ДанныеТаймера, Ид, ТекДата)
	...
	УчетВремениКлиентСервер.УстановитьВидимостьКомандУчетаВремени(Форма, ДанныеТаймера, Ид);
