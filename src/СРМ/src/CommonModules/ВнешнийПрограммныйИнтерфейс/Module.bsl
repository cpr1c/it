...
Функция ОписаниеЗадачиПростое(Задача) Экспорт
	Описание = Новый Структура;
	Описание.Вставить("Тема", Задача.Тема);
	Описание.Вставить("Номер", Задача.Номер);
	Описание.Вставить("Дата", Задача.Дата);
	...
	Описание.Вставить("ДатаИзменения", Задача.ДатаИзменения);
	Описание.Вставить("ДатаНачалаРаботПоЗадаче", Задача.ДатаНачалаРаботПоЗадаче);
	Описание.Вставить("Проект", ВнешнийПрограммныйИнтерфейс.ОписаниеПроекта(Задача.Проект));
	Описание.Вставить("Автор", ВнешнийПрограммныйИнтерфейс.ОписаниеПользователя(Задача.Автор));
	Описание.Вставить("Исполнитель", ВнешнийПрограммныйИнтерфейс.ОписаниеПользователя(Задача.Исполнитель));
	Описание.Вставить("ОценкаТрудозатрат", Задача.ОценкаТрудозатрат);
	Описание.Вставить("ОценкаТрудозатратИсполнителя", Задача.ОценкаТрудозатратИсполнителя);
	Если ЗначениеЗаполнено(Задача.РодительскаяЗадача) Тогда
		Описание.Вставить("РодительскаяЗадача", Задача.РодительскаяЗадача.Номер);
	...
	Описание.Вставить("СодержаниеФормат", ОбщегоНазначения.ИмяЗначенияПеречисления(Задача.СодержаниеФормат));
	Описание.Вставить("СодержаниеHTML", Задача.СодержаниеHTML);
	Описание.Вставить("Статус", ВнешнийПрограммныйИнтерфейс.ОписаниеСтатуса(Задача.Статус));
	Описание.Вставить("КатегорияЗакрытия", ВнешнийПрограммныйИнтерфейс.ОписаниеКатегорииЗадачи(Задача.КатегорияЗакрытия));
	Описание.Вставить("ВидОплаты", ВнешнийПрограммныйИнтерфейс.ОписаниеВидОплаты(Задача.ВидОплаты));
	Описание.Вставить("Трудозатраты", УправлениеЗадачами.ПолучитьТрудозатратыПоЗадаче(Задача.Ссылка));
	Возврат Описание;
КонецФункции
...
Функция ОписаниеОтчетаПоРаботамПартнеру(ОтчетПоРаботам, ВключатьДанныеПоЗадачам) Экспорт
	Если не ЗначениеЗаполнено(ОтчетПоРаботам) Тогда
		Возврат Неопределено;
	КонецЕсли;
	НовыйЭлемент = Новый Структура;
	НовыйЭлемент.Вставить("Идентификатор", Строка(ОтчетПоРаботам.Ссылка.УникальныйИдентификатор()));
	НовыйЭлемент.Вставить("Номер", ОтчетПоРаботам.Номер);
	НовыйЭлемент.Вставить("Дата", ОтчетПоРаботам.Дата);
	НовыйЭлемент.Вставить("Партнер", ОписаниеПартнера(ОтчетПоРаботам.Партнер));
	НовыйЭлемент.Вставить("Комментарий", ОтчетПоРаботам.Комментарий);
	НовыйЭлемент.Вставить("Выставлен", ОтчетПоРаботам.Выставлен);
	НовыйЭлемент.Вставить("ПериодРегистрации", ОтчетПоРаботам.ПериодРегистрации);
	НовыйЭлемент.Вставить("Утвержден", ОтчетПоРаботам.Статус = Перечисления.СтатусыОтчетовПоРаботамПартнеру.Утвержден);
	
	Если Не ВключатьДанныеПоЗадачам Тогда
		Возврат НовыйЭлемент;
	КонецЕсли;
	
	НовыйЭлемент.Вставить("Задачи", Новый Массив);
	
	Для Каждого СтрокаЗадачи Из ОтчетПоРаботам.Задачи Цикл
		ОписаниеСтрокиЗадач = Новый Структура;
		ОписаниеСтрокиЗадач.Вставить("Задача", ОписаниеЗадачиПростое(СтрокаЗадачи.Задача));
		ОписаниеСтрокиЗадач.Вставить("Исполнитель", ОписаниеПользователя(СтрокаЗадачи.Исполнитель));
		ОписаниеСтрокиЗадач.Вставить("Ответственный", ОписаниеПользователя(СтрокаЗадачи.Ответственный));
		ОписаниеСтрокиЗадач.Вставить("ДатаВключенияВОтчет", СтрокаЗадачи.ДатаВключенияВОтчет);
		ОписаниеСтрокиЗадач.Вставить("ВидОплаты", ОписаниеВидОплаты(СтрокаЗадачи.ВидОплаты));
		ОписаниеСтрокиЗадач.Вставить("КоличествоЧасов", СтрокаЗадачи.КоличествоЧасов);
		
		НовыйЭлемент.Задачи.Добавить(ОписаниеСтрокиЗадач);
	КонецЦикла;
