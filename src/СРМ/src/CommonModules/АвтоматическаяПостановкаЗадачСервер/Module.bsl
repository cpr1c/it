...
Процедура АвтоматическаяПоставновкаЗадач() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НастройкаАвтоформирования.ПапкаЭлектроннойПочтыДляПоискаПисем<>Выборка.ПапкаЭлектронногоПисьма И ЗначениеЗаполнено(Выборка.ПапкаЭлектронногоПисьма) Тогда
			Продолжить;
		КонецЕсли;
...
Функция НайтиСуществующуюЗадачу(ТемаПисьма) Экспорт
	...
	РезультатПоиска=РегулярныеВыраженияКлиентСервер.НайтиСовпадения(ТемаПисьма,"\[*#([\d]+)*\]*",КомпонентаРегВыражения);	
...
Функция ТекстСообщенияЭлектроннойПочтыДляЗадачи(СообщениеЭлектроннойПочты,НастройкаАвтоматическогоФормированияЗадач)
...
Функция КонтактСообщенияЭлектроннойПочты(СообщениеЭлектроннойПочты,НастройкаАвтоматическогоФормированияЗадач) 
	Если ЗначениеЗаполнено(СообщениеЭлектроннойПочты.ОтправительКонтакт) И ТипЗнч(СообщениеЭлектроннойПочты.ОтправительКонтакт)=Тип("СправочникСсылка.ФизическиеЛица") Тогда
		Возврат СообщениеЭлектроннойПочты.ОтправительКонтакт;
	КонецЕсли;
	
	Возврат КонтактПоЭдресуИПредставлениюПолучателя(СообщениеЭлектроннойПочты.ОтправительАдрес,СообщениеЭлектроннойПочты.ОтправительПредставление);
КонецФункции

Процедура ЗаполнитьКопииПолучателейВСобытииЗадачиПоЭлектронномуПисьму(СобытиеОбъект,СообщениеЭлектроннойПочты,НастройкаАвтоматическогоФормированияЗадач)
	...
	Для Каждого СтрокаПолучателя Из СообщениеЭлектроннойПочты.ПолучателиПисьма Цикл
		...
		НоваяСтрока=СобытиеОбъект.ПолучателиКопий.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока,СтрокаПолучателя);
		
		НоваяСтрока.Контакт=КонтактПоЭдресуИПредставлениюПолучателя(СтрокаПолучателя.Адрес,СтрокаПолучателя.Представление);
...
Процедура СкопироватьФайлыИзЭлектронногоПисьмаВСобытиеПоЗадаче(Событие,СообщениеЭлектроннойПочты,НастройкаАвтоматическогоФормированияЗадач)
	//Копируем файлы 
	...
	Если ТипЗнч(МассивПрисоединенныхФайлов)=Тип("Массив") Тогда
		ДЛя Каждого ТекВложение ИЗ МассивПрисоединенныхФайлов Цикл
			...
			Если ТипЗнч(Событие)=Тип("ДокументОбъект.Задача") Тогда
				...
				ТекстHTML=Событие.СодержаниеHTML;
			Иначе
				ПараметрыФайла.Вставить("ВладелецФайлов",Событие.Задача);
				ТекстHTML=Событие.ТекстСообщенияHTML;
			КонецЕсли;
			Если ЗначениеЗаполнено(ТекВложение.ИДФайлаЭлектронногоПисьма) И СтрНайти(ТекстHTML,ТекВложение.ИДФайлаЭлектронногоПисьма) Тогда
...
Процедура СоздатьНовуюЗадачуПоСообщениюЭлектроннойПочты(СообщениеЭлектроннойПочты,НастройкаАвтоматическогоФормированияЗадач)
	...
	НоваяЗадача.КонтактОбращения=КонтактСообщенияЭлектроннойПочты(СообщениеЭлектроннойПочты,НастройкаАвтоматическогоФормированияЗадач);
	НоваяЗадача.Автор=АвторСобытияПоПисьмуЭлектроннойПочты(СообщениеЭлектроннойПочты,НастройкаАвтоматическогоФормированияЗадач);
	...
	НоваяЗадача.Содержание=ТекстСообщенияЭлектроннойПочтыДляЗадачи(СообщениеЭлектроннойПочты,НастройкаАвтоматическогоФормированияЗадач);
	НоваяЗадача.СодержаниеHTML=ТекстСообщенияHTMLЭлектроннойПочтыДляЗадачи(СообщениеЭлектроннойПочты,НастройкаАвтоматическогоФормированияЗадач);
	
	ЗаполнитьКопииПолучателейВСобытииЗадачиПоЭлектронномуПисьму(НоваяЗадача,СообщениеЭлектроннойПочты,НастройкаАвтоматическогоФормированияЗадач);
	НоваяЗадача.Записать(РежимЗаписиДокумента.Проведение);
	
	СкопироватьФайлыИзЭлектронногоПисьмаВСобытиеПоЗадаче(НоваяЗадача,СообщениеЭлектроннойПочты,НастройкаАвтоматическогоФормированияЗадач);
КонецПроцедуры

Процедура ДобавитьКомментарийКЗадачеПоСообщениюЭлектроннойПочты(ТекущаяЗадача,СообщениеЭлектроннойПочты,НастройкаАвтоматическогоФормированияЗадач) 
	НовыйКомментарий=Справочники.КомментарииЗадач.СоздатьЭлемент();
	НовыйКомментарий.Задача=ТекущаяЗадача;
	НовыйКомментарий.Автор=АвторСобытияПоПисьмуЭлектроннойПочты(СообщениеЭлектроннойПочты,НастройкаАвтоматическогоФормированияЗадач);
	НовыйКомментарий.ТекстСообщения=ТекстСообщенияЭлектроннойПочтыДляЗадачи(СообщениеЭлектроннойПочты,НастройкаАвтоматическогоФормированияЗадач);
	НовыйКомментарий.ТекстСообщенияHTML=ТекстСообщенияHTMLЭлектроннойПочтыДляЗадачи(СообщениеЭлектроннойПочты,НастройкаАвтоматическогоФормированияЗадач);
	...
	ЗаполнитьКопииПолучателейВСобытииЗадачиПоЭлектронномуПисьму(НовыйКомментарий,СообщениеЭлектроннойПочты,НастройкаАвтоматическогоФормированияЗадач);
	
	НовыйКомментарий.Записать();
	
	СкопироватьФайлыИзЭлектронногоПисьмаВСобытиеПоЗадаче(НовыйКомментарий,СообщениеЭлектроннойПочты,НастройкаАвтоматическогоФормированияЗадач);
КонецПроцедуры
...
Процедура ОбработатьПисьмоЭлектроннойПочты(СообщениеЭлектроннойПочты,НастройкаАвтоматическогоФормированияЗадач) 
	...
	Если ЗначениеЗаполнено(ТекущаяЗадача) Тогда
		ДобавитьКомментарийКЗадачеПоСообщениюЭлектроннойПочты(ТекущаяЗадача,СообщениеЭлектроннойПочты,НастройкаАвтоматическогоФормированияЗадач);
	Иначе
		СоздатьНовуюЗадачуПоСообщениюЭлектроннойПочты(СообщениеЭлектроннойПочты,НастройкаАвтоматическогоФормированияЗадач);
	КонецЕсли;
