...
Процедура АвтоматическаяПоставновкаЗадач() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		НастройкаАвтоформирования=СоответствиеНастроек[Выборка.УчетнаяЗапись];//СправочникСсылка.НастройкиАвтоматическогоФормированияЗадач
...
Процедура ОбработатьПисьмоЭлектроннойПочты(СообщениеЭлектроннойПочты, НастройкаАвтоматическогоФормированияЗадач)
	Если НастройкаАвтоматическогоФормированияЗадач.ЗаблокированныеАдреса.Найти(СообщениеЭлектроннойПочты.ОтправительАдрес,
																			   "АдресЭлектроннойПочты") <> Неопределено Тогда
		Возврат;
