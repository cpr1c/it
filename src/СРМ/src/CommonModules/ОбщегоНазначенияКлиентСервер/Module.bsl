...
#Если ВебКлиент Тогда
	Возврат ОбщегоНазначенияВызовСервера.ПрочитатьДанныеJSON(СтрокаJSON);
#Иначе
...
#Если ВебКлиент Тогда
	Возврат ОбщегоНазначенияВызовСервера.ЗаписатьДанныеJSON(СтруктураДанных);
#Иначе
...
//время 1с  из unixtime
Функция Дата1CИзUnixTime(UnixTime) Экспорт
	Если Не ЗначениеЗаполнено(UnixTime) Тогда
		Возврат '00010101';
	КонецЕсли;	
		
	Возврат '19700101' + UnixTime; 
КонецФункции 

// Возвращает время в формате UNIX из 1С
Функция ДатаUnixTimeИз1С(ПараметрДата) Экспорт 
