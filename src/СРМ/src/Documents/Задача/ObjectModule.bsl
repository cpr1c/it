...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Движения.ВыработкаИсполнителей.Записывать = Истина;
	
	Если Статус.Вид = Перечисления.ВидыСтатусовЗадач.Закрыта Тогда
		ВыработкаПоОсновномуИсполнителю = ОценкаТрудозатрат - ДополнительныеИсполнители.Итог("ОценкаТрудозатрат");

		Движение = Движения.ВыработкаИсполнителей.Добавить();
		Движение.Период = ДатаЗакрытия;
		Движение.Задача = Ссылка;
		Движение.Исполнитель = Исполнитель;
		Движение.Количество = ВыработкаПоОсновномуИсполнителю;

		Для Каждого СтрокаДопИсполнителя Из ДополнительныеИсполнители Цикл
			Движение = Движения.ВыработкаИсполнителей.Добавить();
			Движение.Период = ДатаЗакрытия;
			Движение.Задача = Ссылка;
			Движение.Исполнитель = СтрокаДопИсполнителя.Исполнитель;
			Движение.Количество = СтрокаДопИсполнителя.ОценкаТрудозатрат;
		КонецЦикла;
	КонецЕсли;
КонецПроцедуры
