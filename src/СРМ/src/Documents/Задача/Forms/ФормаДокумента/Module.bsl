...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	ОбновитьСостояниеЭлементовТекущегоТаймера();
...
Процедура ВводТрудозатратЧасовРегулирование(Элемент, Направление, СтандартнаяОбработка)
	УправлениеЗадачамиКлиент.ПолеЧасовРегулирование(ЭтотОбъект, "ВводТрудозатратЧасов", Направление, СтандартнаяОбработка);
КонецПроцедуры
...
Процедура ИсторияПоЗадачеДокументСформирован(Элемент)
	ПутьКДаннымБезЛишнего=РедакторКомментарияКлиентСервер.ПолучитьПутьКДаннымБезЛишнего("Объект.Содержание",
		Объект.СодержаниеФормат);
	УправлениеЗадачамиКлиент.ОбновитьИсториюЗадачи(Объект.Ссылка, Элемент, ЭтотОбъект,
...
&НаКлиенте
Процедура СтартоватьТаймер(Команда)
	УчетВремениКлиент.СоздатьИСтартоватьТаймерИзФормыПредмета(ЭтотОбъект);
КонецПроцедуры
&НаКлиенте
Процедура УдалитьТаймер(Команда)
	УчетВремениКлиент.УдалитьТаймер(ЭтотОбъект, ИдентификаторТаймера);
КонецПроцедуры

&НаКлиенте
Процедура ОстановитьТаймер(Команда)
	УчетВремениКлиент.ОстановитьТаймер(ЭтотОбъект, ИдентификаторТаймера);
КонецПроцедуры

&НаКлиенте
Процедура ЗафиксироватьТрудозатратыПоТаймеру(Команда)
	Если ИдентификаторТаймера = Неопределено Или Не ЗначениеЗаполнено(ИдентификаторТаймера) Тогда
		Возврат;
	КонецЕсли;

	Если УчетВремениКлиентСервер.ТаймерАктивенПоДатам(ТаймерДатаНачала, ТаймерДатаОкончания) Тогда
		УчетВремениКлиент.ОстановитьТаймер(ЭтотОбъект, ИдентификаторТаймера);
	КонецЕсли;

	ДанныеТаймера=УчетВремениВызовСервера.ДанныеТаймера(ИдентификаторТаймера);
...
Процедура ДобавитьНаблюдателяЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	УправлениеЗадачами.ДобавитьНаблюдателя(Объект.Ссылка, Результат);
	ЗаполнитьНаблюдателей();
КонецПроцедуры
...
&НаСервере
Процедура ОбновитьСостояниеЭлементовТекущегоТаймера()
	ИдентификаторТаймера=УчетВремени.ТаймерПоПредмету(Объект.Ссылка);
	Если ИдентификаторТаймера = Неопределено Тогда
		ТаймерАктивен=Ложь;
	Иначе
		ДанныеТаймера=УчетВремени.ДанныеТаймера(ИдентификаторТаймера);
		ТаймерАктивен=УчетВремениКлиентСервер.ТаймерАктивен(ДанныеТаймера);

		ТаймерДатаНачала=ДанныеТаймера.ДатаНачала;
		ТаймерДатаОкончания=ДанныеТаймера.ДатаОкончания;
		ТаймерЧасов=ДанныеТаймера.Часов;
	КонецЕсли;
	Элементы.СтартоватьТаймер.Видимость=Не ТаймерАктивен;
	Элементы.ОстановитьТаймер.Видимость=ТаймерАктивен;

	Если ТаймерАктивен Тогда
		ТаймерРежимФиксации=Ложь;
	КонецЕсли;

	РасчитатьПродолжительностьТаймера(ЭтотОбъект);
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура РасчитатьПродолжительностьТаймера(Форма)
#Если Клиент Тогда
	ЭлементыПриВводеКоторыхНеОбновлятьТаймер=Новый Массив;
	ЭлементыПриВводеКоторыхНеОбновлятьТаймер.Добавить("ВводТрудозатратКомментарий");
	ЭлементыПриВводеКоторыхНеОбновлятьТаймер.Добавить("ТекстНовогоКомментария");

	НельзяОбновлятьТаймер=Ложь;
	Для Каждого ТекИмяЭлемента Из ЭлементыПриВводеКоторыхНеОбновлятьТаймер Цикл
		ТекЭлемент=Форма.Элементы[ТекИмяЭлемента];

		Если ЗначениеЗаполнено(ТекЭлемент.ТекстРедактирования) И ТекЭлемент.ТекстРедактирования
			<> Форма[ТекИмяЭлемента] Тогда
			НельзяОбновлятьТаймер=Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;

	Если НельзяОбновлятьТаймер Тогда
		Возврат;
	КонецЕсли;
#КонецЕсли

	Если Форма.ИдентификаторТаймера = Неопределено Тогда
		Форма.ПродолжительностьТаймера=0;
	Иначе
		Форма.ПродолжительностьТаймера=УчетВремениКлиентСервер.ПродолжительностьТаймераВсего(Форма.ТаймерДатаНачала,
			Форма.ТаймерДатаОкончания, Форма.ТаймерЧасов);
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Процедура Подключаемый_УчетВремени_ОбновитьЭлементыТаймеров(ИдентификаторИзменяемогоТаймера) Экспорт
	Если ИдентификаторИзменяемогоТаймера <> ИдентификаторТаймера Тогда
		Возврат;
	КонецЕсли;
	ОбновитьСостояниеЭлементовТекущегоТаймера();
КонецПроцедуры
