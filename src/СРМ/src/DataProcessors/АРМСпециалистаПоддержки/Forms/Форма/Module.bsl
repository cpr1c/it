...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УстановитьВидимостьВспомогательныхЭлементовПанелиДействий();

	ПолеHTMLТаймеров=ПолучитьОбщийМакет("ПолеТаймеров").ПолучитьТекст();
КонецПроцедуры
...
Процедура ПолеHTMLТаймеровОбработчикСобытия(Событие)
	...
	Если ИмяСобытия="OPEN_CONTEXT_URL" Тогда
		ОбщегоНазначенияКлиент.ОткрытьНавигационнуюСсылку(ДанныеСобытия);
	КонецЕсли;
КонецПроцедуры
...
&НаСервере
Процедура УстановитьВидимостьВспомогательныхЭлементовПанелиДействий()
	КоличествоТаймеров=Элементы.ГруппаПанельДействийТаймеры.ПодчиненныеЭлементы.Количество();

	Элементы.СвернутьПанельДействий.Видимость=КоличествоТаймеров > 0;
КонецПроцедуры
