...
Процедура ЗадачиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли Поле.Имя = "ЗадачиДополнительныеИсполнителиПредставление" Тогда
		СтандартнаяОбработка = Ложь;

		ДопИсполнители = Новый Массив;
		Для Каждого Стр Из СтрокаТЧ.ДополнительныеИсполнители Цикл
			СтруктураИсполнителя = Новый Структура;
			СтруктураИсполнителя.Вставить("Исполнитель", Стр.Исполнитель);
			СтруктураИсполнителя.Вставить("ОценкаТрудозатрат", Стр.ОценкаТрудозатрат);

			ДопИсполнители.Добавить(СтруктураИсполнителя);
		КонецЦикла;

		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("ОценкаТрудозатрат", СтрокаТЧ.ОценкаТрудозатрат);
		ПараметрыФормы.Вставить("ДополнительныеИсполнители", ДопИсполнители);
		Если СтрокаТЧ.ДобавленаВОтчет Тогда
			ПараметрыФормы.Вставить("ТолькоПросмотр", Истина);
		КонецЕсли;

		ПараметрыОповещения = Новый Структура;
		ПараметрыОповещения.Вставить("ТекДанные", СтрокаТЧ);

		ОткрытьФорму("Обработка.ФормированиеОтчетовПоРаботеПартнерам.Форма.ФормаРедактированияДополнительныхИсполнителей",
					 ПараметрыФормы,
					 Элемент,
					 ,
					 ,
					 ,
					 Новый ОписаниеОповещения("ЗадачиДополнительныеИсполнителиПредставлениеНажатиеЗавершение",
											  ЭтотОбъект,
											  ПараметрыОповещения),
...
&НаКлиенте
Процедура ЗадачиДополнительныеИсполнителиПредставлениеНажатиеЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ТекДанные.ДополнительныеИсполнители.Очистить();
	
	Для Каждого Стр Из Результат Цикл
		НоваяСтрока = ТекДанные.ДополнительныеИсполнители.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, Стр);
	КонецЦикла;
	
	УстановитьПредставлениеДополнительныхИсполнителейВСтрокеЗадач(ТекДанные);
...
Процедура ПрименитьИзмененияЗадачи(СтрокаДерева, СоответствиеДанныхОтчетовПоРаботам)
	Если СтрокаДерева.Изменено Тогда
		...
		ЗадачаОбъект.ДополнительныеИсполнители.Очистить();
		Для Каждого Стр Из СтрокаДерева.ДополнительныеИсполнители Цикл
			НоваяСтрока = ЗадачаОбъект.ДополнительныеИсполнители.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, Стр);
		КонецЦикла;
...
Процедура СформироватьОтчетыПоРаботе(СоответствиеДанныхОтчетовПоРаботам)
	Для Каждого КлючЗначение Из СоответствиеДанныхОтчетовПоРаботам Цикл
		...
		Для Каждого ИдентификаторСтрокиДерева Из КлючЗначение.Значение Цикл
			...
			Для Каждого СтрДопИсполнителей Из СтрокаДерева.ДополнительныеИсполнители Цикл
				НоваяСтрокаДопИсполнителей = ОтчетПоРаботамОбъект.ДополнительныеИсполнители.Добавить();
				НоваяСтрокаДопИсполнителей.КлючСтроки = НоваяСтрока.КлючСтроки;
				НоваяСтрокаДопИсполнителей.Исполнитель = СтрДопИсполнителей.Исполнитель;								
				НоваяСтрокаДопИсполнителей.КоличествоЧасов = СтрДопИсполнителей.ОценкаТрудозатрат;								
			КонецЦикла;
		КонецЦикла;
...
&НаСервере
Процедура ВывестиДеревоЗадачПоДеревуЗапроса(ДеревоЗапроса, ДеревоЗадач, СтрокиЗадач, Уровень = 0)
	Для Каждого СтрокаДерева Из ДеревоЗапроса.Строки Цикл
		...
		Если ЗначениеЗаполнено(СтрокаДереваЗадач.Задача) Тогда
			СтрокиЗадач.Вставить(СтрокаДереваЗадач.Задача, СтрокаДереваЗадач);
			УстановитьПредставлениеДополнительныхИсполнителейВСтрокеЗадач(СтрокаДереваЗадач);
			
		КонецЕсли;
		
		ВывестиДеревоЗадачПоДеревуЗапроса(СтрокаДерева, СтрокаДереваЗадач, СтрокиЗадач, Уровень + 1);
...
Процедура ЗаполнитьДеревоЗадач()
	...
	СтрокиЗадач = Новый Соответствие;
	
	ВывестиДеревоЗадачПоДеревуЗапроса(Дерево, Задачи, СтрокиЗадач);
	
	МассивЗадач = Новый Массив;
	Для Каждого КлючЗначение Из СтрокиЗадач Цикл
		МассивЗадач.Добавить(КлючЗначение.Ключ);
	КонецЦикла;
	
	Запрос = Новый Запрос;
	Запрос.Текст=
	"ВЫБРАТЬ
	|	ЗадачаДополнительныеИсполнители.Ссылка КАК Ссылка,
	|	ЗадачаДополнительныеИсполнители.Исполнитель,
	|	ЗадачаДополнительныеИсполнители.ОценкаТрудозатрат
	|ИЗ
	|	Документ.Задача.ДополнительныеИсполнители КАК ЗадачаДополнительныеИсполнители
	|ГДЕ
	|	ЗадачаДополнительныеИсполнители.Ссылка В (&Ссылка)
	|ИТОГИ
	|ПО
	|	Ссылка";
	Запрос.УстановитьПараметр("Ссылка", МассивЗадач);
	
	ВыборкаЗадачи = Запрос.Выполнить().Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	Пока ВыборкаЗадачи.Следующий() Цикл
		СтрокаЗадачи = СтрокиЗадач[ВыборкаЗадачи.Ссылка];
		
		ВыборкаДетали = ВыборкаЗадачи.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		Пока ВыборкаДетали.Следующий() Цикл
			НоваяСтрока = СтрокаЗадачи.ДополнительныеИсполнители.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, ВыборкаДетали);
		
			УстановитьПредставлениеДополнительныхИсполнителейВСтрокеЗадач(СтрокаЗадачи);
		КонецЦикла;
	КонецЦикла;
...
Процедура УстановитьПредставлениеДополнительныхИсполнителейВСтрокеЗадач(СтрокаЗадач)
	КоличествоДопИсполнителей = СтрокаЗадач.ДополнительныеИсполнители.Количество();
	
	Если Не ЗначениеЗаполнено(КоличествоДопИсполнителей) Тогда
		СтрокаЗадач.ДополнительныеИсполнителиПредставление = "Нет";
	Иначе
		СтрокаЗадач.ДополнительныеИсполнителиПредставление = ""
															 + Формат(СтрокаЗадач.ДополнительныеИсполнители.Итог("ОценкаТрудозатрат"),
																	  "ЧДЦ=2;")
															 + " ч.";
	КонецЕсли;
КонецПроцедуры
