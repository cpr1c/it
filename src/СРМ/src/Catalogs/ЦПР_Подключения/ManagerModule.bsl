...
Процедура ПриЗаполненииОграниченияДоступа(Ограничение) Экспорт
	...
	|	И ЗначениеРазрешено(ВидПодключения)";
...
Функция ПолучитьАдресПодключения(Адрес,Порт) Экспорт 
	Возврат Адрес+?(ЗначениеЗаполнено(Порт),":"+Формат(Порт,"ЧГ=0"),"");
КонецФункции

Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	Представление = СтрШаблон("%1 (%2)", Данные.Наименование, Данные.Владелец.Партнер);
...
Процедура УстановитьОформлениеСписка(Знач СписокПодключенийИлиЕгоУсловноеОформление) Экспорт
	...
	ЭлементОтбораДанных.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ПредварительноеПодключение");
	...
	ЭлементЦветаОформления.Значение = Метаданные.ЭлементыСтиля.ПодключениеСПредшествующимПодключениемФон.Значение;
